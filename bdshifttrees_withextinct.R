...
frac<-c(1) # only rate shifts (no mass extinction)
...
tree<-sim.rateshift.taxa(n=taxa,numbsim = sim,lambda = lambda,mu=mu,frac = frac, times = times, complete = TRUE)

write.nexus(tree,file = "simbdtreelist_withextinct.nex")
